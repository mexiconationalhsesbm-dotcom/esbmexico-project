    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[600px] m-4">
      <div className="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{taskName}</p>
          <p className="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">
            This action cannot be undone. The task and all its submissions will be permanently deleted.
          </p>
        </div>

        {/* Error Message */}
        {error && (
          <div className="mt-4 flex items-start space-x-2 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-900/30 dark:text-red-300">
            <span>{error}</span>
          </div>
        )}
